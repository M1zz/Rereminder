...
알림 개수 히스토그램 (2.1.2 추가 — "주로 몇 개를 쓰나"):
`alertRuns.0` `alertRuns.1` … `alertRuns.5` `alertRuns.6plus` — 타이머를 시작할 때마다
그때 건 알림 개수 칸을 1 올린 값(0인 칸은 보내지 않는다).
...
최대값 하나로는 **한 번 5개를 걸어 본 사람과 늘 5개를 거는 사람이 같은 값으로 보인다.**
그래서 실행마다 개수를 세어(`alertRuns.*`) 두 가지로 읽는다 — 단위가 다르므로 한 차트에
겹치지 않고 화면에서 피커로 갈아 끼운다:

- **실행 기준**(`AlertRunBucket.runs`): 모든 설치의 실행 수 합계 = 이 앱이 실제로 돌아가는 모양
- **사람 기준**(`AlertRunBucket.installs`): 설치마다 최빈값 하나씩 = 그 개수를 자기 기본값으로 삼은 사람

`UsageInsights.typicalAlertCount`는 그 설치의 최빈값이고, **동률이면 작은 쪽**을 고른다
(결제 판단에 쓰는 값이라 과대평가가 과소평가보다 비싸다). 히스토그램이 없는 스냅샷(2.1.2 이전)은
어느 칸도 올리지 않는다 — 통계 화면의 "이 값을 보내온 설치" 비율이 그 커버리지다.
계산은 전부 순수 함수 + 테스트(`UsageInsightsTests`).
