...
- **무료 사용자에게도 ①을 한 번 띄운다**(`maxFreeProposals` = 1, `markProposed(asFree:)`).
  문구가 처음부터 "Pro 로 기억해 두기"라고 말하고, 누르면 다이얼에 올린 뒤 페이월을 연다.
  ⚠️ 무료는 콜드 런치에 다이얼이 기본값이라 **마지막으로 쓴 설정**(`lastUsedSetup`)으로 판단한다 —
  다이얼로 보면 무료에서는 반복이 영영 잡히지 않는다. 자세한 건 아래 "앱이 기억한다" 절.
...
- **전날 저녁부터 그날까지 앱을 열면 그 설정을 다이얼에 올린다**(`RememberPitch.eveBookingToPrepare`,
  Pro·무료 모두 — 시트가 그렇게 약속했다). 알림을 탭하지 않고 직접 열어도 같다.
  한 예약에 한 번만 올린다(그 뒤에 바꾼 설정을 덮으면 안 된다). 무료 사용자에게는
  "이게 Pro 가 매번 하는 일"이라는 알림창이 붙는다.

### "앱이 기억한다"를 무료 사용자에게 보여 주는 세 자리 (RememberPitch)
`Rereminder/Modules/RememberPitch.swift`(판정) + `Views/Components/RememberRecallLine.swift`.

Pro 가 파는 한 문장이 "앱이 설정을 기억한다"인데 무료 사용자는 그 가치를 **겪을 길이 없었다** —
콜드 런치에 다이얼이 조용히 기본값으로 돌아가니 "원래 그런 앱"으로 읽혔다.
그래서 **방금 겪은 불편에 이름을 붙이는** 자리를 셋 둔다(2026-09-16):

- **① 다시 연 순간의 한 줄**: 다이얼이 기본값이고 마지막 설정이 기본값과 다르면 원 아래(템플릿 바
  위)에 "지난번엔 25:00, 알림 3개였어요. Pro 는 이걸 기억해 둡니다." **모달이 아니다.**
  하루 한 번, 통틀어 7일(`maxRecallDays`). 다이얼을 바꾸거나 결제하면 물러난다.
  ⚠️ 눌러도 설정을 올려 주지 **않는다** — 올려 주면 Pro 가 파는 일을 공짜로 하게 된다.
- **② 반복 감지**: 위 `RepeatDetector` 절. 무료에게 한 번.
- **③ 다음 자리 전날**: 위 "다음 자리 예약" 절. 체험은 이날 한 번이다.
- **④ 손으로 다시 맞춘 순간**: 콜드 런치에 기본값으로 돌아간 다이얼을 사용자가 **직접** 지난번 설정에
  맞추고 1초 머물면 같은 자리에 "다시 손으로 맞추셨네요. Pro 는 이걸 다이얼에 기억해 둡니다."
  앱이 대신 올려 준 경우(시간대 제안·전날·반복 제안)는 세지 않는다(`reentryTarget = nil`).
- ⚠️ **Pro 언급은 설치당 예산 하나로 센다 — `ProMention`.** 자리마다 따로 세면 하나씩은 절제돼 보여도
  합쳐서 한 사람이 열 번 넘게 듣는다. 넛지는 처음 몇 번이 일을 하고, 알아들은 뒤에는 잔소리다.
  통틀어 **3번**, 자리마다 **한 번**, 한 번 말하면 **3일** 쉰다, 권유를 **한 번 눌렀거나 페이월을 2번
  봤으면 멈춘다.** 전날에 설정을 올려 주는 **약속**은 예산과 무관하고 말만 빠진다.
  사용자가 잠긴 기능을 눌러 여는 페이월은 대답이지 넛지가 아니라 예산을 쓰지 않는다.
  **새로 Pro 를 먼저 꺼내는 자리를 만들면 반드시 이 예산을 거칠 것.** 테스트: `ProMentionTests` (11개)
- **문구 실험**(`ProMention.CopyVariant`): 세션 모드를 써 본 사람만 반씩 나눠 한쪽엔 "앱이 기억한다",
  다른 쪽엔 "구간 이름·대본까지 저장"을 앞세운다. 갈래는 **처음 권유를 그릴 때 정해져 바뀌지 않는다.**
  바꾸는 건 문구뿐 — 페이월·예산·빈도는 같아야 비교가 된다. 판정은 스냅샷
  `flag.pitchCopySession`/`flag.pitchCopyRemember` × `flag.isPaid`(뷰어 "권유" 섹션).
  ⚠️ Pro 는 구간 이름·대본을 **템플릿으로 저장**할 뿐 다시 열 때 복원하지 않는다 — 문구에 "돌아온다"고 쓰지 말 것.
- ⚠️ **한 번에 하나만.** 순서는 ③ → 시간대 제안 → ② → ① (`TimerUnifiedView.setupOnAppear`).
  ③이 다이얼을 채웠으면 나머지는 전부 건너뛴다.
- 원격 플래그 `RereminderFlag.rememberPitchEnabled` 로 끌 수 있다(CloudKit `RemoteFlags` 필드,
  없으면 켬). 꺼도 ③의 **설정 올려 주기**는 약속이라 계속하고 권유 문구만 빠진다.
- 지표: `remember_pitch_shown:{recall|repeat|eve}` → `remember_pitch_tapped:{…}` →
  `paywall_converted:unlimitedTemplates`. 세 자리 중 어디가 통하는지를 가른다.
- 테스트: `RereminderTests/RememberPitchTests.swift` (11개)
...
- ⚠️ **템플릿에 무료 몫이 없다.** 예전의 `freeTemplateLimit = 3` 은 삭제됐다 — **개수로 다시
  나누지 말 것.** 예외는 날짜 하나뿐이다 — **개편 전에 만든 템플릿은 무료에서도 불러온다**
  (`LegacyFreeNotice.isPreChangeTemplate`, 2.2.8). 그 사람들이 무료로 쓰던 것이고, 새로 저장하는 건 여전히 Pro 다. 저장·불러오기 자체가 파는 물건이다. 체험(5+5)도 없다: 기억은 쌓여야 값이
...
- ⚠️ **못 하는 일을 권하지 않는다 — 권할 때는 처음부터 Pro 라고 말한다.** 온보딩의 "템플릿 저장"
  버튼은 내지 않는다(대신 그런 것이 있다는 한 줄만). 반복 저장 제안은 무료에게 **한 번**, 문구가
  "Pro 로 기억해 두기"인 형태로만 뜬다 — 저장 버튼을 보여 놓고 누르는 순간 막는 일은 여전히 없다
  (위 "앱이 기억한다" 절).
...
### 평생 무료(그랜드파더링)도 래치다 — 재설치로 사라지면 안 된다
⚠️ **2.2.8 이전에는 그랜드파더링 자격이 `UserDefaults` 에만 있었다.** 앱을 지웠다 깔면 자격이
사라지고, 재판정에 쓰는 흔적(`selectedThemeID`·`pushEnabled`·`timerCompletionCount`)도 같은 곳에
있어 함께 지워져 **영영 돌아오지 않았다.** 2026-09-18 에 "평생 Pro 를 쓰고 있었는데 사라졌다"는
제보로 확인된 경로다. 결제 기록과 **같은 급으로** 다룬다:

- 자격은 **UserDefaults + Keychain 두 벌**로 남긴다(`grantGrandfather`). `isGrandfathered` 는
  둘 중 하나만 있어도 참이고, Keychain 에서 읽으면 UserDefaults 를 되살려 둔다.
- `isProUser` 는 **창단 후원자 표식**(Keychain `rereminder.founder`)도 근거로 본다 — 그 표식은
  결제했거나 평생 무료를 약속받은 사람에게만 주어지고 앱을 지워도 남는다.
- 앱을 켤 때(`StoreManager.init`)와 **구매 복원**에서 `recoverLostEntitlementIfPossible()` 이
  돈다. 설정 > Pro 아래 **"Pro를 쓰고 있었는데 사라졌나요?"** 줄에서 사용자가 직접 부를 수도 있다.
- ⚠️ **옛 흔적 검사를 다시 돌리지 말 것.** 그 흔적(테마를 골랐나·완주한 적 있나)은 신규 사용자도
  며칠이면 만든다 — Pro 도입 버전으로 막 올라온 **그 한 번**에만 뜻이 있는 신호다. 다시 돌리면
  새로 깐 사람에게 평생 무료가 나간다. 복구는 **Keychain 에 남은 기록만** 본다.
- ⚠️ Keychain 마저 지운 사람(기기 초기화·2.2.4 이전에 재설치)은 코드로 되살릴 수 없다 —
  설정의 그 줄이 피드백으로 안내하고, 사람이 프로모션 코드로 돌려준다.
- 테스트: `StorePurchaseLatchTests` 의 재설치·창단 후원자 표식·근거 없음 세 가지.
...
### 개편 전 무료 사용자 — 잃은 것을 숨기지 않는다 (LegacyFreeNotice)
`Rereminder/Modules/LegacyFreeNotice.swift`(판정) + `Views/LegacyFreeNoticeView.swift`(화면).

파는 축을 옮기며(2.2.2, 첫 배포 2.2.4) **그 전의 무료 사용자는 얻은 것과 잃은 것이 함께 생겼다** —
예비 알림 무제한을 얻었고, 템플릿 3개 무료 저장·불러오기와 마지막 설정 복원을 잃었다.
창단 후원자·그랜드파더링에게는 안내가 있었지만 **이 사람들에게만 아무 설명이 없었다.**
화면은 얻은 것 → Pro 로 옮긴 것 → **원래 쓰던 것은 그대로 두었다** → 왜, 순서로 사실만 적는다.
- **설명만 하고 돌려주지 않으면 "잃으셨습니다, 결제하세요"로 읽힌다.** 그래서 2.2.8 부터 개편 전에
  만든 템플릿은 무료에서도 불러온다(`isPreChangeTemplate` → `TemplateQuickBar.canLoad`).
  경계는 같은 `windowClosedAt` 이라 새로 설치한 사람의 시드는 여전히 잠긴다.

- ⚠️ 판정은 **날짜가 붙은 흔적**으로만 한다. 경계는 `FoundingSupporter.windowClosedAt`.
  ① 창이 이번 실행에 닫혔는데 이미 타이머를 건 적이 있다 ② 옛 알림 한도 시절에만 쌓이던 키
  (`legacyOnlyKeys`) ③ 경계보다 먼저 만든 템플릿(시드 포함) ④ 경계보다 먼저 남은 기록.
- ⚠️ **창 닫기·판정은 `seedTemplatesIfNeeded` 보다 앞이어야 한다**(`TimerUnifiedView.setupOnAppear`).
  시드가 먼저 심기면 신규 설치자의 시드가 ③에 걸려 **새 사용자 전원이 "잃었다"는 말을 듣는다.**
- 흔적은 과거의 것이라 **한 번만 본다**(`checkedKey`). 참이면 기억해 두고 설정 > Pro 줄 아래
  "달라진 점"으로 다시 연다. Pro·창단 후원자에게는 띄우지 않는다.
- 혜택 변경 안내(이것·`FounderWelcomeView`)가 떠 있으면 **다른 안내는 전부 양보한다**
  (`isShowingPlanChangeNotice`). ⚠️ 의견 요청 알림창이 먼저 뜨면 시트가 가려 **뜨지 않는다** —
  시뮬레이터에서 실제로 그랬다. 그래서 `FeedbackNudge` 판정은 이 안내 **뒤에** 있다.
- ⚠️ "지워지지 않았다·결제하면 그대로 돌아온다"는 두 가지가 지켜질 때만 참이다 — 무료에서도
  `persistLastUsedConfig` 가 저장하고, 잠긴 템플릿을 지우지 않는 것. 바꾸면 문구도 고칠 것.
- 지표: `remember_pitch_shown:legacy` → `remember_pitch_tapped:legacy`(보조 버튼 = 페이월).
- 테스트: `RereminderTests/LegacyFreeNoticeTests.swift` (10개)
...
- **FeedbackNudge** (`Rereminder/Modules/FeedbackNudge.swift`): 앱이 먼저 의견을 묻는 경로
  (5회째 실행 → 이후 20회 간격, "다시 보지 않기"=6개월 유예, 만족도 게이트에 양보).
  통계가 "어디서 떨어지는지"를 말해 준다면 이유는 이 경로로 들어온다.
  ⚠️ **`launchCount` 는 프로세스 실행당 1 씩만 오른다** — 홈에 다녀오는 것은 세지 않는다.
  그래서 하루 한 번 여는 사용자에게 이 숫자는 곧 **날수**다. 10/40 이던 시절에는 첫 질문까지
  열흘, 다음까지 또 40일이 걸려 **대부분이 그전에 앱을 지웠다**(피드백이 거의 없던 이유).
  ⚠️ 반대로 더 줄이지도 말 것 — 양보 순서를 지키는 안내가 이미 여럿이라(기기 질문·반복 감지·
  창단 후원자·다음 자리 예약) 더 자주 물으면 앱을 열 때마다 무언가 뜨는 앱이 된다.
- ⚠️ **피드백 창구를 Help 안에 두지 말 것.** 2.2.4 까지 "Send Feedback" 은 설정 > **Help**
  섹션의 여섯 번째 줄이었는데, 그 섹션은 부제까지 "첫 실행 안내가 전부 여기 있습니다"라
  **사용법 문서로 읽힌다** — 불만이 있는 사람은 도움말을 열지 않는다. 지금은 Help 앞에
  독립된 **Feedback** 섹션이 있고, Instagram DM 도 같이 옮겼다(둘 다 "개발자에게 말 거는" 줄).
- ⚠️ **워치에는 피드백 창구를 만들 수 없다** — LeeoKit 의 `Package.swift` 플랫폼이
  iOS/macOS/macCatalyst 뿐이라 `LeeoFeedbackView` 가 워치 타겟에서 아예 컴파일되지 않는다.
  만들려면 LeeoKit 에 watchOS 지원을 먼저 넣어야 한다.
...
`scripts/check_localization.py` 의 `LANGS`. 하나라도 빠지면 게이트와 실제가 갈라진다.

- ⚠️ **번체를 간체의 자동 변환으로 만들지 말 것.** 글자만 바꾸면 대만 사용자에게는 어색한 대륙
  용어가 그대로 남는다. 실제로 갈리는 말들:
  설정 `设置`/`設定` · 템플릿 `模板`/`範本` · 위젯 `小组件`/`小工具` ·
  잠금화면 `锁定屏幕`/`鎖定畫面` · 홈 화면 `主屏幕`/`主畫面` · 다이나믹 아일랜드 `灵动岛`/`動態島` ·
  Live Activity `实时活动`/`即時動態` · 단축어 앱 `快捷指令`/`捷徑` · 메뉴 막대 `菜单栏`/`選單列` ·
  저장 `保存`/`儲存` · 기본값 `默认`/`預設` · 탭 `点按`/`點一下` · Digital Crown `数码表冠`/`數位錶冠`.
  따옴표도 다르다 — 간체는 `“ ”`, 번체는 `「 」`.
- **용어 고정**: 세션 모드 = `分段模式`(구간을 나눠 운영한다는 뜻을 그대로 쓴다. 대륙·대만 공통),
  구간(Section) = `分段`, 예비 알림(Pre-alert) = `提前提醒`, 타이머 = `计时器`/`計時器`,
  대본(Script) = `讲稿`/`講稿`. **이 다섯은 화면 여러 곳에 흩어져 있으므로 새 문구를 쓸 때도 같은
  말을 쓸 것** — 갈라지면 같은 기능이 두 이름을 갖는다.
- **앱 이름은 `Rereminder` 그대로 둔다**(일본어와 같은 선택). 중국어 이름을 새로 만들면
  App Store 리스팅·검색어·이 문서의 고유명사가 전부 갈라진다. 바꾸려면 세 타겟의
  `InfoPlist.xcstrings` 와 `app_name`·`app_name_notification`·`app_name_pro` 를 함께 고칠 것.
- ⚠️ **`LOCALES` 에 적어도 App Store 페이지에 그 언어 현지화가 없으면 릴리즈노트가 올라가지
  않는다.** App Store Connect ▸ 앱 정보 ▸ 현지화에서 **간체 중국어·번체 중국어를 사람이 추가**
  해야 한다(`docs/OPERATIONS_CHECKLIST.md`). 인앱결제 표시 이름도 별개다 — 위 "앱 이름" 절 참고.
- ⚠️ **웹 페이지 넷(`web/index.html`, `docs/index.html`·`support.html`·`privacy.html`)은 아직
  한국어·영어뿐이다.** 앱만 중국어가 되고 소개·지원 페이지는 영어로 남아 있다.
- 검증: `python3 scripts/check_localization.py` 로 구멍을 잡고, 빌드한 번들에서 실제로 확인한다.
  ```bash
  plutil -p <번들>/zh-Hant.lproj/Localizable.strings | grep '"Settings"'   # 設定 여야 한다
  ```
...
`### App Store (日本語)` · `### 앱스토어 (중국어 간체)` · `### 앱스토어 (중국어 번체)`)을
읽어 그대로 싣는다. **이 절이 비어 있으면 배포가 막힌다**(커밋 제목으로
...
- 한 릴리즈에 파일이 다섯 벌이므로 내용이 바뀌면 **다섯 벌을 함께** 고친다.
...
- **개편 전 무료 사용자에게 달라진 점 안내**(`LegacyFreeNotice` + `LegacyFreeNoticeView`) +
  개편 전에 만든 템플릿은 무료에서도 불러오기. 설정 > Pro 줄 아래 "달라진 점"으로 다시 연다
- **Pro 언급 예산**(`ProMention`): 설치당 3번·자리마다 1번·3일 간격·알아들으면 멈춤.
  다시 연 순간 한 줄은 7일 → 1번
...
- ⚠️ **평생 무료가 재설치로 사라지던 문제 수정** — 자격을 Keychain 에도 남기고, 창단 후원자
  표식으로도 되살린다. 설정에 "Pro를 쓰고 있었는데 사라졌나요?" 줄 추가 (제보 2026-09-18)
